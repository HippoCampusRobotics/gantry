...
import rclpy
import yaml
from gantry_msgs.msg import MotorPosition, MotorStatus, MotorVelocity
...
class GridPositionControl(Node):
    def __init__(self, node_name):
        ...
        self.start_srv = self.create_service(
            Trigger, '~/start', self.serve_start
        )
        ...
        self.meas_time_pub = self.create_publisher(
            BoolStamped, 'measurement_active', 1
        )
        ...
        self.control_timer = self.create_timer(
            timer_period_sec=(1 / 50), callback=self.on_control_timer
        )
        ...
        self.get_logger().info(
            ...
            f'{round(self.estimated_total_time / 60.0)} min '
        )
    ...
    def on_motor_status(self, msg: MotorStatus, index: int):
        ...
        if all(self.position_reached):
            if not self.all_positions_reached:
                ...
                self.get_logger().info(
                    'First time all motors reached waypoint!'
                )
                ...
                if dt < (
                    self.delay_before_measurement + self.wait_time_measurement
                ):
                    ...
                    if (
                        ...
                        self.get_logger().info(
                            'Now measuring at waypoint'
                            + (
                                f' {self.current_waypoint_index + 1} of '
                        ...
                        self.get_logger().info(
                            (
                                ...
                                f'{round(remaining_time / 60.0)} min'
                            ),
                            throttle_duration_sec=self.wait_time_measurement,
                        )
                    return
                    ...
                    if not self.updated_waypoint_index:
                        ...
                        waypoint = self.waypoint_list[idx]
                        self.get_logger().info(
                            (
                                'Finished at waypoint '
                                f'{waypoint} with index {idx}'
                            )
                        )
                        ...
                        if self.current_waypoint_index < len(
                            ...
                            self.get_logger().info(
                                (
                                    'Moving on to waypoint '
                                    f'{waypoint} with index {idx}'
                                )
                            )
                        else:
                            self.get_logger().info(
                                'Finished all waypoints. Stopping...'
                            )
                            self.stop()
    ...
    def get_estimated_time_to_end(self, waypoint_index: int) -> float:
        ...
        remaining_time = (
            self.wait_time_measurement + self.delay_before_measurement
        ) * num_waypoints
        for wp in range(waypoint_index, len(self.waypoint_list) - 1):
            max_time = [None] * len(self.axes)
            for i in range(len(self.axes)):
                distance_i = (
                    self.waypoint_list[wp + 1][i] - self.waypoint_list[wp][i]
                )
                max_time[i] = distance_i / self.max_speeds[i]
            remaining_time += (
                max(max_time) + 0.5
            )  # some extra to account for axis to start moving
    ...
    def stop(self):
        ...
        self.current_waypoint_index = 0
        self.get_logger().info(
            'Resetting waypoint index. Call start service to start again.'
        )
